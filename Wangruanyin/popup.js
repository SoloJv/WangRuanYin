...
chrome.storage.local.get(
  ['pinyinEnabled', 'selectionEnabled', 'targetLang', 'hskMode', 'hskDisabledLevels', 'hskEnabled'],
  (result) => {
    const pinyinToggle = document.getElementById('togglePinyin');
    const selectionToggle = document.getElementById('toggleSelection');
    const hskToggle = document.getElementById('toggleHsk');
    ...
    hskToggle.checked = result.hskEnabled === true;                        // default OFF
    ...
    function updateHskLegend(mode, disabledLevels) {
      const maxLevel = mode === 'hsk3' ? 9 : (mode === 'hsk2' ? 6 : 0);
    ...
    document.querySelectorAll('input[name="hskMode"]').forEach((radio) => {
      radio.addEventListener('change', () => {
        ...
        // Selecting "Off" turns the standalone highlight off as well.
        if (mode === 'off' && hskToggle.checked) {
          hskToggle.checked = false;
          chrome.storage.local.set({ hskEnabled: false });
          getActiveTab((tabId) => {
            sendMessage(tabId, { action: 'set_hsk_enabled', enabled: false });
          });
        }
...
// --- HSK colours master toggle (standalone, no translation) ----------
    hskToggle.addEventListener('change', () => {
      const newState = hskToggle.checked;
      // Turning it on should always have an actual HSK version selected, never
      // "Off", otherwise nothing would be highlighted.
      if (newState) {
        const checkedRadio = document.querySelector('input[name="hskMode"]:checked');
        if (!checkedRadio || checkedRadio.value === 'off') {
          const hsk2 = document.querySelector('input[name="hskMode"][value="hsk2"]');
          if (hsk2) hsk2.checked = true;
        }
      }
      const mode = ((document.querySelector('input[name="hskMode"]:checked') || {}).value) || 'off';
      chrome.storage.local.set({ hskEnabled: newState, hskMode: mode }, () => {
        getActiveTab((tabId) => {
          // Note: we only send set_hsk_enabled here — NOT set_hsk_mode, because
          // set_hsk_mode would trigger the pinyin annotation / translation and
          // undo the instant colour highlight we just applied.
          sendMessage(tabId, { action: 'set_hsk_enabled', enabled: newState, mode });
        });
      });
      showStatus(newState ? 'HSK highlighting ON' : 'HSK highlighting OFF');
    });

// --- On popup open, sync colours to the active tab ------------------
    if (hskMode !== 'off') {
      getActiveTab((tabId) => {
        chrome.tabs.sendMessage(tabId, { action: 'hsk_sync', mode: hskMode, levels: hskDisabledLevels, enabled: hskToggle.checked }, (resp) => {
          if (chrome.runtime.lastError || !(resp && resp.applied)) {
            ensureContentInjected(tabId, () => {
              chrome.tabs.sendMessage(tabId, { action: 'set_hsk_enabled', enabled: hskToggle.checked }, () => {
